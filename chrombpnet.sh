...
if [ -f "${MACS2_DIR}/union_final.narrowPeak" ] && [ -f "${MACS2_DIR}/all_peaks.narrowPeak" ]; then
    echo "  > [Info] Union peaks already exist (${MACS2_DIR}/union_final.narrowPeak). Skipping merge and downsample."
else
    ...
    if [ "$ENABLE_DOWNSAMPLE" = true ]; then
        ...
        if [ "$UNION_BASE" -le "$TARGET_PEAKS" ]; then
            ...
            for peak_file in "${MACS2_DIR}"/*_peaks.narrowPeak; do
...
if [ ! -f "${MACS2_DIR}/union_filter_negatives.bed" ]; then
    ...
    echo "  > [Info] Nonpeaks already prepared (${MACS2_DIR}/union_filter_negatives.bed). Skipping."
fi
echo "Step 2 Done."
