...
{
if $MYSQL -uroot -proot -e "SHOW DATABASES;" 2> /dev/null > /dev/null; then
    true
else
    echo2 "ERROR:"
...
function mysqlDbSetup ()
{
    ...
    waitKey
    $MYSQL -uroot -proot -e 'CREATE DATABASE IF NOT EXISTS customTrash;'
    $MYSQL -uroot -proot -e 'CREATE DATABASE IF NOT EXISTS hgcentral;'
    $MYSQL -uroot -proot -e 'CREATE DATABASE IF NOT EXISTS hgTemp;'
    $MYSQL -uroot -proot -e 'CREATE DATABASE IF NOT EXISTS hgFixed;' # empty db needed for gencode tracks
    downloadFile http://$HGDOWNLOAD/admin/hgcentral.sql | $MYSQL -uroot -proot hgcentral
    # the blat servers don't have fully qualified dom names in the download data
    $MYSQL -uroot -proot hgcentral -e 'UPDATE blatServers SET host=CONCAT(host,".cse.ucsc.edu");'
    ...
    $MYSQL -uroot -proot -e "GRANT SELECT, INSERT, UPDATE, DELETE, FILE, "\
...
"IDENTIFIED BY 'genome';"
    ...
    $MYSQL -uroot -proot -e "GRANT FILE on *.* TO browser@localhost IDENTIFIED BY 'genome';" 
    
    #   Read only access to genome databases for the browser CGI binaries
    $MYSQL -uroot -proot -e "GRANT SELECT, CREATE TEMPORARY TABLES on "\
"*.* TO readonly@localhost IDENTIFIED BY 'access';"
    $MYSQL -uroot -proot -e "GRANT SELECT, INSERT, CREATE TEMPORARY TABLES on hgTemp.* TO "\
"readonly@localhost IDENTIFIED BY 'access';"
    
    # Readwrite access to hgcentral for browser CGI binaries to keep session state
    $MYSQL -uroot -proot -e "GRANT SELECT, INSERT, UPDATE, "\
...
"IDENTIFIED BY 'update';"
    ...
    $MYSQL -uroot -proot -e "GRANT SELECT,INSERT,UPDATE,DELETE,CREATE,DROP,ALTER,INDEX "\
"on customTrash.* TO ctdbuser@localhost IDENTIFIED by 'ctdbpassword';"
    ...
    $MYSQL -uroot -proot -e 'CREATE DATABASE IF NOT EXISTS hg18'
...
function downloadMinimal
{
    ...
    for db in $DBS; do
       ...
       for track in $hideTracks; do
            mysql -uroot -proot $db -e 'UPDATE trackDb set visibility=0 WHERE tableName="'$track'"'
        done

       for track in $notSearchTables; do
            mysql -uroot -proot $db -e 'DELETE from hgFindSpec WHERE searchTable="'$track'"'
        done
    done
    echo2 
