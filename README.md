...
> data type (-m) is also required for aggregation (-a).
...
./cellranger.sh -m atac -r 200 -t 24 /path/to/all_samples/
# run cellranger aggr
./cellranger.sh -m atac -r 200 -t 24 -a /path/to/cellranger/count/output/with/all/samples/
```

> Sequencing depth normalization is not recommended in most situations. For RNA data, merge the data matrix in Seurat/scanpy is a more efficient way.
